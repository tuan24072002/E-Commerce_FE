            ...
            <div className=' flex items-center gap-4 md:gap-6 overflow-x-scroll scrollbar-none transition-all' ref={scrollElement}>
                ...
                <i className='absolute right-0 top-[50%] transform translate-y-[50%] border-red-200 hover:bg-black hover:rounded-full hover:text-white bg-white rounded-full flex justify-center items-center p-0 md:p-1 text-black cursor-pointer' onClick={() => scrollRight()}><IoIosArrowForward size={'1.5em'} /></i>
                {
                    isLoading ? loadingList.map((pro, index) => {
                        return (
                            <div className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] bg-white rounded-lg shadow cursor-pointer' key={index} >
                                <div className='bg-slate-200 h-48 animate-pulse p-4 min-w-[120px] md:min-w-[145px] group rounded-tl-lg rounded-bl-lg' style={{ transition: '0.5s linear' }}>

                                </div>
                                <div className='p-3 grid w-full gap-3 animate-pulse'>
                                    <div className='font-medium text-base md:text-lg text-ellipsis line-clamp-1 bg-slate-200 h-4'></div>
                                    <div className='capitalize text-slate-500 bg-slate-200 h-4'></div>
                        ...
                        data && data.length > 0 && data.map((pro, index) => {
                            return (
                                <Link to={`/product/${pro._id}`} className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px]  bg-white rounded-lg shadow cursor-pointer' key={index} onClick={() => ScrollToTop()}>
